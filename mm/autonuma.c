...
unsigned long autonuma_flags __read_mostly =
	...
	|(1<<AUTONUMA_SCHED_TASK_RESET_FLAG)
	|(1<<AUTONUMA_SCHED_MM_RESET_FLAG)
...
static int numa_hinting_fault_memory_follow_cpu(struct task_struct *p,
						...
						bool new_pass)
{
	if (!last_nid_set(p, page, this_nid))
		return page_nid;
	if (!PageLRU(page))
		return page_nid;
...
SYSFS_ENTRY(task_reset, AUTONUMA_SCHED_TASK_RESET_FLAG);
SYSFS_ENTRY(mm_reset, AUTONUMA_SCHED_MM_RESET_FLAG);
...
int alloc_mm_autonuma(struct mm_struct *mm)
{
	...
	if (!mm_autonuma)
		goto out;
	if (autonuma_sched_mm_reset() || !mm->mm_autonuma)
