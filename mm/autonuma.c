				   ...
				   unsigned long mm_numa_fault_pass)
{
	...
	task_autonuma->task_false_sharing = false;
						 ...
						 int numpages)
{
	...
	struct mm_autonuma *mm_autonuma = p->mm->mm_autonuma;
	unsigned long *task_numa_fault = task_autonuma->task_numa_fault;
	unsigned long *mm_numa_fault = mm_autonuma->mm_numa_fault;
	unsigned long mm_numa_fault_pass;

	/* prevent sched_autonuma_balance() to run on top of us */
	local_bh_disable();

	mm_numa_fault_pass = ACCESS_ONCE(mm_autonuma->mm_numa_fault_pass);
	if (unlikely(task_autonuma->task_numa_fault_pass !=
		     mm_numa_fault_pass))
		cpu_follow_memory_pass(p, task_autonuma, task_numa_fault,
				       mm_autonuma, mm_numa_fault,
				       mm_numa_fault_pass);
 ...
 */
static bool should_migrate_page(struct task_struct *p,
				struct page *page, int this_nid, int page_nid)
{
	bool ret = true;
	...
	VM_BUG_ON(this_nid >= nr_node_ids);
	VM_BUG_ON(autonuma_last_nid < -1);
	VM_BUG_ON(autonuma_last_nid >= nr_node_ids);
	if (autonuma_last_nid != this_nid) {
		/* pending rewrite removing the two bytes per page of course */
		if (page_nid == autonuma_last_nid) {
			ret = false;
			p->task_autonuma->task_false_sharing = true;
		}
		ACCESS_ONCE(page_autonuma->autonuma_last_nid) = this_nid;
	}
	if (p->task_autonuma->task_false_sharing)
		ret = false;
	return ret;
}
...
static inline int task_autonuma_reset_size(void)
{
	...
	return task_autonuma_size() -
		(int)((char *)(&task_autonuma->TASK_AUTONUMA_RESET_START) -
		      (char *)task_autonuma);
}

static void __task_autonuma_reset(struct task_autonuma *task_autonuma)
{
	memset(&task_autonuma->TASK_AUTONUMA_RESET_START, 0,
...
static inline int mm_autonuma_reset_size(void)
{
	...
	return mm_autonuma_size() -
		(int)((char *)(&mm_autonuma->MM_AUTONUMA_RESET_START) -
		      (char *)mm_autonuma);
}

static void mm_autonuma_reset(struct mm_autonuma *mm_autonuma)
{
	memset(&mm_autonuma->MM_AUTONUMA_RESET_START, 0,
	       mm_autonuma_reset_size());
}
...
static int knumad_do_scan(void)
{
	...
	for (; vma && progress < pages_to_scan; vma = vma->vm_next) {
		...
		if (vma->vm_flags & (VM_SPECIAL | VM_INSERTPAGE | VM_MIXEDMAP |
				     VM_SAO | VM_HUGETLB)) {
			progress++;
			continue;
		}
...
static int knuma_scand(void *none)
{
	...
	for (;;) {
		...
		if (unlikely(!knuma_scand_data.mm)) {
			...
			/*
			 * If the pages aren't in the LRU, we can't
			 * migrate them so drain the pagevecs before
			 * starting a new pass.
			 */
			lru_add_drain_all();
		}
...
static struct attribute *autonuma_attr[] = {
	...
	/* migrate start */
	&migrate_interval_millisecs_attr.attr,
...
int alloc_mm_autonuma(struct mm_struct *mm)
{
	...
	mm_autonuma->mm = mm;
	spin_lock_init(&mm_autonuma->mm_numa_fault_lock);
no_numa:
	err = 0;
out:
	return err;
