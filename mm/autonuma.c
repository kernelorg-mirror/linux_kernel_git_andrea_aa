...
void numa_hinting_fault(struct page *page, int numpages)
{
	 ...
	 * "mm". That is ok, all we care about is to account the
	 * "page_nid" access on the current->task_autonuma, even if
	 * the page belongs to a different "mm".
	 */
...
static int start_knuma_scand(void)
{
	...
	knuma_scand_data.mm_numa_fault_tmp = kmalloc(mm_autonuma_fault_size(),
						     GFP_KERNEL);
