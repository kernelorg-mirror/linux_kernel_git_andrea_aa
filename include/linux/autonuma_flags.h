...
enum autonuma_flag {
	...
	/*
	 * This flag define if the task/mm_autonuma statistics should
	 ...
	 * reduces the number of NUMA hinting page faults by a large
	 * factor potentially saving some CPU, but it reduces the
	 * accuracy of the task_autonuma statistical information
	 * (there is no change in accuracy of the mm_autonuma
	 * statistical information). This flag can be toggled through
	 * sysfs at runtime.
	 *
	 * This flag is only relevant for regular pages, with THP the
	 * NUMA hinting page faults only happen at the pmd level and
	 * there is no reduction in accuracy of task_autonuma with
	 * THP.
	 *
	 * Default set.
	 */
