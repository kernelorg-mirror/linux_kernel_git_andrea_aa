...
enum autonuma_flag {
	...
	/*
	 * These two flags, defines if the task/mm_autonuma statistics
	 * should be inherithed from the parent task/process or
	 * instead if they should be cleared at every fork/clone. The
	 * task/mm_autonuma statistics are always cleared across
	 * execve and there's no way to disable that.
	 */
	AUTONUMA_SCHED_TASK_RESET_FLAG,
	AUTONUMA_SCHED_MM_RESET_FLAG,
	/*
	 * If set this tells knuma_scand to create pmd_numa instead of
	 * pte_numa. This reduces the number of NUMA hinting page
	 * faults by a large factor potentially saving some CPU, but
	 * it reduces the accuracy of the task_autonuma statistical
	 * information (there is no change in accuracy of the
	 * mm_autonuma statistical information). This flag can be
	 * toggled through sysfs at runtime.
	 *
	 * This flag is only relevant for regular pages, with THP only
	 * pmd_numa are created and there is no reduction in accuracy
	 * of task_autonuma with THP.
	 *
	 * Default set.
	 */
	AUTONUMA_SCAN_PMD_FLAG,
	/*
	 * This flag changes the semantics of the mm_autonuma
	 * statistical information. If set it would account for only
	 * the memory accessed cumulatively by all threads of the
	 * process over the last knuma_scand pass. If clear it
	 * accounts all process memory that is young according to
	 * ptes/pmds or mmu notifier.
	 *
	 * Default not set.
	 */
	AUTONUMA_SCAN_USE_WORKING_SET_FLAG,
	/*
	 * If set knuma_migrated will wakeup exactly in the middle of
	 * each knuma_scand pass, regardless of how many pages have
	 * been already queued. If not set knuma_migrated will wakeup
	 * as soon as the number of pages in the migration LRU reached
	 * a certain threshold.
	 *
	 * Default not set.
	 */
...
static inline bool autonuma_sched_task_reset(void)
{
	return test_bit(AUTONUMA_SCHED_TASK_RESET_FLAG,
			&autonuma_flags);
}

static inline bool autonuma_sched_mm_reset(void)
{
	return test_bit(AUTONUMA_SCHED_MM_RESET_FLAG,
			&autonuma_flags);
}
