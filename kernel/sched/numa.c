 ...
 * provides an net and unidirectional benefit in increasing the
 * system wide NUMA convergence.
 *
 * The tasks that are the closest to "fully converge" are given the
 * maximum priority in being located in their "best node".
 *
 * "Full convergence" is achieved when all memory of the task is fully
 * CPU local.
 *
 * The "best node" is the NUMA node that recently had the maximum
 * amount of memory accesses from the task, after excluding all nodes
 * that aren't already busy running other tasks that are closer to
 * converge in them than the current one.
 *
 * To find how close is a task to converge we use weights. These
 ...
 *
 * If the weights are equal no switch will happen.
 *
 ...
 *
 * If both checks succeeds it guarantees that we found a way to
 * unidirectionally improve the system wide NUMA
 * convergence. Unidirectional means that the same checks will not
 * succeed again on those same two tasks, after the task exchange, so
 * there is no risk of ping pong.
 *
 * If a task exchange can happen because the two checks succeeds, we
 ...
 * quoted code).
 *
 * CFS has to become NUMA affinity aware through the introduction of
 * sched_autonuma_can_migrate_task(). CFS searches CPUs in the task's
 * task_selected_nid first, when it needs to find idle CPUs during
 * idle balancing or tasks to pick during load balancing.
 *
 * The task's task_selected_nid is the node selected by
 * __sched_autonuma_balance() when it migrates the current task to the
 * selected_cpu in the selected_nid during the task exchange.
 *
 * Once a task has been moved to another node, closer to the much of
 ...
 *
 * Only if the two candidate tasks for exchange belong to the same
 * process, the mm_autonuma information would be identical, so we use
 * the task_autonuma information so the thread will follow their own
 * memory locality and they will not necessarily try to converge on
 * the same node. False sharing however will be accounted for, so the
 * more false sharing the more they will try to converge in the same
 * node.
 *
 ...
 * memory as in tmpfs, but it refers to CLONE_VM instead. The only
 * sure way to avoid the risk of NUMA false sharing is to schedule all
 * threads accessing the same memory in the same node.
 *
 * This algorithm might be expanded to take all runnable processes
 * into account later.
 *
 * This algorithm is run by every CPU in the context of the
...
void __sched_autonuma_balance(void)
{
	 ...
	 * Avoid the task to bounce around randomly by setting the
	 * task_selected_nid.
	 */
	...
	/*
	 * If the NUMA node this thread, and the process as a whole,
	 * has the most NUMA faults is this NUMA node, there is no
	 * need to migrate this thread somewhere else.
	 */
	...
	for_each_online_node(nid) {
		...
		for_each_cpu_and(cpu, cpumask_of_node(nid), allowed) {
			...
			task_tot = rq->curr->task_autonuma->task_numa_fault_tot;
			if (!mm_tot || !task_tot) {
			...
			task_weight = rq->curr->task_autonuma->task_numa_fault[nid];
			raw_spin_unlock_irq(&rq->lock);
