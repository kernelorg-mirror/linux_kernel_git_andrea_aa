...
static void set_task_selected_nid(struct task_autonuma *task_autonuma,
				  int task_selected_nid)
{
	if (ACCESS_ONCE(task_autonuma->task_selected_nid) != task_selected_nid)
		ACCESS_ONCE(task_autonuma->task_selected_nid) =
			task_selected_nid;
}
...
static int autonuma_balance_cpu_stop(void *data)
{
	...
	if (ACCESS_ONCE(p->task_autonuma->task_selected_nid) !=
	    cpu_to_node(dst_cpu))
		goto out_unlock;
...
void __sched_autonuma_balance(void)
{
	...
	if (!autonuma_enabled()) {
		set_task_selected_nid(task_autonuma, -1);
		return;
	}
	...
	set_task_selected_nid(task_autonuma, this_nid);

	p_mm = p->mm;
	mm_autonuma = p_mm->mm_autonuma;
	...
	/*
	 * The task is already running on the node where it does the most
	 * memory accesses. There is no need for NUMA balancing.
	 */
	...
	/* task migration follows */
	BUG_ON(this_nid == selected_nid);
	BUG_ON(!other_task);

	rq = cpu_rq(this_cpu);
	other_rq = cpu_rq(selected_cpu);
	if (ACCESS_ONCE(rq->autonuma_balance) ||
	    ACCESS_ONCE(other_rq->autonuma_balance) ||
	    ACCESS_ONCE(other_rq->curr) != other_task)
		return;

	autonuma_printk("%p %d - %dto%d - %dto%d - %ld %ld %ld - %s\n",
			p_mm, p->pid, this_nid, selected_nid,
			this_cpu, selected_cpu,
			s_w_other, s_w_nid, s_w_this_nid,
			s_w_type_thread ? "thread" : "process");
	...
	migrate_other_task = rq->nr_running <= 1;
	raw_spin_unlock_irq(&rq->lock);
	...
	set_task_selected_nid(task_autonuma, selected_nid);
	 ...
	 * callback once the migration work is finished.
	 */
	raw_spin_lock_irq(&other_rq->lock);
	/*
	 ...
	 */
	if (other_rq->curr != other_task || other_rq->autonuma_balance) {
		raw_spin_unlock_irq(&other_rq->lock);
		return;
	}
	/*
	 * Set task_selected_nid while the other_task can't be freed
	 * from under us. This means if a load balancing happens the
	 * other task will be more likely be picked than "p" even if
	 * migrate_other_task is false and we leave it there.
	 */
	set_task_selected_nid(other_task->task_autonuma, this_nid);
	if (!migrate_other_task)
		raw_spin_unlock_irq(&other_rq->lock);
	else {
		other_rq->autonuma_balance = true;
		/* take the pin on the task struct before dropping the lock */
		get_task_struct(other_task);
		raw_spin_unlock_irq(&other_rq->lock);

		other_rq->autonuma_balance_dst_cpu = this_cpu;
		other_rq->autonuma_balance_task = other_task;

		/* Do the actual migration. */
		stop_one_cpu_nowait(selected_cpu,
				    autonuma_balance_cpu_stop, other_rq,
				    &other_rq->autonuma_balance_work);
		/* other_task is a dangling pointer here */
		raw_spin_lock_irq(&other_rq->lock);
		if (other_rq->autonuma_balance && other_rq->curr == other_task)
			resched_task(other_task);
		raw_spin_unlock_irq(&other_rq->lock);
	}
				     ...
				     enum cpu_idle_type idle)
{
	...
	/*
	 * Exclude CPU_NEWLY_IDLE events to avoid crossing the NUMA
	 * affinity boundaries for too short idle periods.
	 */
